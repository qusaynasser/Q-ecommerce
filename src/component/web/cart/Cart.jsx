...
export default function Cart() {
  const { getCartContext, removeCartContext, clearAllCart, loading, increaseCartQuantity, decreaseCartQuantity } = useContext(CartContext);
  if (loading) {
  ...
  const incraseQuantity = async (productId) => {
    const result = await increaseCartQuantity(productId);
    return result;
  }

  const decraseQuantity = async (productId) => {
    const result = await decreaseCartQuantity(productId);
    return result;
  }
  let total = 0;
  return (
            ...
            <div className="products" id="products">
              ...
              {data?.products.length ? (data.products.map((product) =>
                <div className="item" key={product._id}>
                  ...
                  <div className="quantity">
                    <button onClick={() => decraseQuantity(product.details._id)}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={16}
                        height={17}
              ...
              )) : <h2>cart is emptay</h2>}

              <button type="button" className="btn btn-danger bg-danger w-25" onClick={clearCart}>Clear Cart</button>
            </div>
